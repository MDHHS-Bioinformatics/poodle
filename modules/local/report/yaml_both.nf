process YAML_BOTH {
  ...
  tuple val(meta), path(ref_eval), path(snptree), path(snpmatrix),
        path(pan_summary), path(pan_roary), path(pan_rtab), path(pan_genedists),
        path(gubtree), path(gubmatrix), path(mashtree), path(mashmatrix), emit: files
  path("*.yaml"), emit: yaml
  ...
  echo "gubtree: \$(basename ${gubtree})"                >> ${prefix}.yaml
  echo "gubmatrix: \$(basename ${gubmatrix})"            >> ${prefix}.yaml
