...
workflow PROCESSCLUSTERPERSPECIES {
    ...
    if (params.mashtree){
        ...
        INPUT_CHECK.out.final_assembly_files
        .filter{meta, assembly -> meta.has_assembly == true}
        .map{ meta, assembly -> tuple([[species:meta.species, cluster_id:meta.cluster_id], assembly]) }
        .groupTuple(by: [0])
        .set{ ch_assemblies }
